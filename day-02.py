...
for d in data:
    diff = [a - b for a, b in pairwise(d)]
    if all(map(lambda x: x <= maxdiff and x >= mindiff, diff)) or all(
        map(lambda x: x >= -maxdiff and x <= -mindiff, diff)
    ):
        safe += 1
print(safe)
...
def is_safe(to_check):
    diff = [a - b for a, b in pairwise(to_check)]
    return all(map(lambda x: x <= maxdiff and x >= mindiff, diff)) or all(
        map(lambda x: x >= -maxdiff and x <= -mindiff, diff)
    )
